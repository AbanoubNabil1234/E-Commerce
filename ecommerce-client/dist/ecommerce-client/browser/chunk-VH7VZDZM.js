...
(function(NotificationType2) {
  NotificationType2[NotificationType2["Order"] = 0] = "Order";
  NotificationType2[NotificationType2["Shipment"] = 1] = "Shipment";
  NotificationType2[NotificationType2["Inventory"] = 2] = "Inventory";
  NotificationType2[NotificationType2["System"] = 3] = "System";
  NotificationType2[NotificationType2["Delivery"] = 4] = "Delivery";
  NotificationType2[NotificationType2["Alert"] = 5] = "Alert";
  NotificationType2[NotificationType2["Promotion"] = 6] = "Promotion";
})(NotificationType || (NotificationType = {}));
var NotificationTypeLabels = {
  [NotificationType.Order]: { en: "Order", ar: "\u0637\u0644\u0628", icon: "\u{1F4E6}", color: "#3B82F6" },
  [NotificationType.Shipment]: { en: "Shipment", ar: "\u0634\u062D\u0646\u0629", icon: "\u{1F69A}", color: "#8B5CF6" },
  [NotificationType.Inventory]: { en: "Inventory", ar: "\u0645\u062E\u0632\u0648\u0646", icon: "\u{1F4CA}", color: "#EF4444" },
  ...
  [NotificationType.Promotion]: { en: "Promotion", ar: "\u0639\u0631\u0636", icon: "\u{1F389}", color: "#EC4899" }
};
...
var NotificationsService = class _NotificationsService {
  ...
  getNotifications(page = 1, pageSize = 20) {
    ...
    return this.http.get(`${environment.apiUrl}/notifications`, { params }).pipe(tap((response) => {
   ...
   */
  getUnreadCount() {
    return this.http.get(`${environment.apiUrl}/notifications/unread-count`).pipe(tap((response) => this.unreadCount.set(response.count)));
  }
  /**
   * Mark notification as read.
   */
  markAsRead(id) {
    return this.http.patch(`${environment.apiUrl}/notifications/${id}/read`, {}).pipe(tap(() => {
      const current = this.notifications();
      const updated = current.map((n) => n.id === id ? __spreadProps(__spreadValues({}, n), { isRead: true, readAt: (/* @__PURE__ */ new Date()).toISOString() }) : n);
   ...
   */
  markAllAsRead() {
    return this.http.patch(`${environment.apiUrl}/notifications/read-all`, {}).pipe(tap(() => {
      const current = this.notifications();
      const updated = current.map((n) => __spreadProps(__spreadValues({}, n), { isRead: true, readAt: (/* @__PURE__ */ new Date()).toISOString() }));
      this.notifications.set(updated);
   ...
   */
  delete(id) {
    return this.http.delete(`${environment.apiUrl}/notifications/${id}`).pipe(tap(() => {
      const current = this.notifications();
   ...
   */
  create(request) {
    return this.http.post(`${environment.apiUrl}/notifications`, request);
  }
  /**
  ...
  startSignalR(token) {
    ...
    this.hubConnection = new HubConnectionBuilder().withUrl(`${environment.apiUrl.replace("/api", "")}/hubs/notifications`, {
      accessTokenFactory: () => {
    ...
    this.hubConnection.start().then(() => console.log("[SignalR] Connected to NotificationHub")).catch((err) => console.error("[SignalR] Connection error:", err));
  }
  /**
  ...
  formatRelativeTime(dateStr) {
    const date = /* @__PURE__ */ new Date(dateStr + (dateStr.endsWith("Z") ? "" : "Z"));
  ...
  showToast(notification) {
    ...
    switch (notification.type) {
      case NotificationType.Order:
        this.toastService.info(message, { title: title || "\u{1F4E6} Order Update" });
        break;
      case NotificationType.Shipment:
        this.toastService.info(message, { title: title || "\u{1F69A} Shipment Update" });
        break;
      case NotificationType.Inventory:
        this.toastService.warning(message, { title: title || "\u{1F4CA} Inventory Alert" });
        break;
      case NotificationType.System:
        this.toastService.info(message, { title: title || "\u{1F514} System" });
        break;
      default:
...
//# sourceMappingURL=chunk-VH7VZDZM.js.map
